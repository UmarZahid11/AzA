<div class="dashboard-content posted-theme">
    <div style="float:right;">
        <?php if ($this->model_signup->hasPremiumPermission()) : ?>
            <div class="">
                <!-- side-large-text -->
                <a class="btn btn-custom" href="<?= l('dashboard/webinar/save/create') ?>">Create webinar</a>
                <a class="btn btn-custom" href="<?= l('dashboard/webinar/attendance') ?>">Attended webinar</a>
            </div>
        <?php endif; ?>
    </div>
    ...
    <h4><?= __('Webinars')?></h4>
    <hr />
    <a href="<?= l(TUTORIAL_PATH . WEBINAR_TUTORIAL) ?>" target="_blank"><i class="fa fa-film"></i> Webinar Tutorial</a>
    <hr />
    <div class="row">
        <div class="col-md-6">
            <small class="line-height-2"><?= record_detail($offset, $webinars, $webinars_count) ?></small>
        </div>
    </div>
    <hr/>
    <table class="style-1">
        ...
        <?php if (isset($webinars) && count($webinars) > 0) : ?>
            <tbody>
                <?php foreach ($webinars as $key => $value) : ?>
                    <tr>
                        <td>
                        ...
                        <td>
                            ...
                            <?php if (isset($value['webinar_start_url']) && $this->model_signup->hasPremiumPermission() && $this->userid == $value['webinar_userid']) : ?>
                                <?php if (in_array($value['webinar_current_status'], [ZOOM_WEBINAR_PENDING, ZOOM_WEBINAR_ENDED])) : ?>
                                    <a href="<?= l('dashboard/webinar/save/update/' .  JWT::encode($value['webinar_id'])) ?>" target="_blank" class="actns" data-toggle="tooltip" data-bs-placement="top" title="<?= __("Edit webinar.") ?>" data-id="<?= $value['webinar_id'] ?>"><i class="fa-regular fa-edit"></i></a>
                                    <a class="actns" href="<?= l('dashboard/webinar/processURL/' . JWT::encode($value['webinar_id']) . '/' . ZOOM_TYPE_START_WEBINAR) ?>" data-toggle="tooltip" data-bs-placement="top" title="<?= __("Start webinar.") ?>" data-id="<?= $value['webinar_id'] ?>"><i class="fa-regular fa-link"></i></a>
                                <?php endif; ?>
                            ...
                            <?php if (isset($value['webinar_join_url']) && $this->model_signup->hasPremiumPermission() && $this->userid !== (int) $value['webinar_userid']) : ?>
                                <?php if($value['webinar_current_status'] == ZOOM_WEBINAR_STARTED): ?>
                                    <?php if ($value['webinar_current_status'] == ZOOM_MEETING_STARTED) : ?>
                                        <a class="actns" href="<?= l('dashboard/webinar/processURL/' . JWT::encode($webinar['webinar_id']) . '/' . ZOOM_TYPE_JOIN_WEBINAR) ?>" data-toggle="tooltip" data-bs-placement="top" title="<?= __("Join webinar.") ?>" data-id="<?= $webinar['webinar_id'] ?>"><i class="fa-regular fa-link"></i></a>
                                    <?php endif; ?>
                                <?php endif; ?>
                            <?php endif; ?>
                            <?php if (isset($value['webinar_start_url']) && $this->model_signup->hasPremiumPermission() && $this->userid == $value['webinar_userid']) : ?>
        ...
        <?php else : ?>
            <table>
                <?= __('No webinars available.') ?>
            </table>
        <?php endif; ?>
    </table>
</div>

<?php if (isset($webinars_count) && ($webinars_count) > 0) : ?>
    <div class="row mt-4">
        <div class="col-lg-12">
